...
class FilterHerosTest {
    ...
    fun orderByProWinsAsc() {
        ...
        for (index in 1 until emissions.size) {
            val prevWinPercentage =
                (emissions[index - 1].proWins.toDouble() / emissions[index - 1].proPick.toDouble() * 100).roundToInt()
                    .toInt()
            val currWinPercentage =
                (emissions[index].proWins.toDouble() / emissions[index].proPick.toDouble() * 100).roundToInt()
                    .toInt()
